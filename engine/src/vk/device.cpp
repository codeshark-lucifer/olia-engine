...
namespace Engine
{
  ...
  EngineDevice::EngineDevice(Platform &plt) : plt{plt}
  {
    createInstance();
    setupDebugMessenger();
    createSurface();
  ...
  void EngineDevice::createInstance()
  {
    ...
    appInfo.pApplicationName = "Olia App";
  ...
  {
    plt.CreateWindowSurface(instance, &surface_);
  }
  ...
  {
    SDL_Window *window = plt.GetWindow();
    Uint32 extensionCount = 0;
    char const *const *extensions = SDL_Vulkan_GetInstanceExtensions(&extensionCount);

    if (!extensions)
    {
      throw std::runtime_error(std::string("Failed to get SDL Vulkan instance extensions: ") + SDL_GetError());
    }

    // Copy the C-style array pointers into a C++ vector
    std::vector<const char *> required(extensions, extensions + extensionCount);

    // If you need validation layers, add the debug messenger extension manually
    if (enableValidationLayers)
    {
